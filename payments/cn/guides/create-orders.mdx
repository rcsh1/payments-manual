...
- 调用 [Create pay-in order](/payments/en/api-references/payment/create-pay-in-order) 直接创建一个支付订单。调用成功后，您将得到应付金额、付款地址等信息。您需要自行搭建前端页面，也可借助 [React SDK / Vue SDK](/payments/cn/guides/payment-toolkit) 引入 Web3 支付能力；
...
[账户与资金分配](/payments/cn/guides/amounts-and-balances#充币归属)中详细介绍了多付、少付和晚付情况下 Cobo 对资金的处理规则，包括：当充币发生在订单已经过期、已达到终态 `Underpaid`，或已处于 `Completed` 状态之后时，该笔资金会被计入开发者账户，而非商户账户。
