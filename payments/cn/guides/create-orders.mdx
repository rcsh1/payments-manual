...
<Tabs>
  <Tab title="使用 Payments API 创建支付订单" icon="code">
    ...
    - **应收币种**（`payable_currency`）： 您计划收取的数字货币币种，目前支持数字货币币种请参考 [支持的币种与公链](/payments/cn/guides/supported-chains-and-tokens)
    - **应收金额**（`payable_amount`）：您计划向付款方收取的数币货币金额，以`payable_currency` 指定的加密货币为单位。该字段为可选字段：
    ...
    - **开发者费用**（`fee_amount`）：如果您是服务多个下游商户的平台机构，需要在您和下游商户之间分配收入，可以通过设置该费用来实现。此费用以`payable_currency` 指定的加密货币为单位。实际分成的开发者费用=开发者费用/应收金额\*实际收款金额，更多信息请参考[账户与资金分配](/payments/cn/guides/amounts-and-balances)。
