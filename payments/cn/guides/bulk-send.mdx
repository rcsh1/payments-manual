---
title: "Bulk Send 集成指南"
---

Bulk Send 用于一次性向多个收款地址批量转账，适用于工资发放、返佣奖励、合作方结算、电商/广告平台商家收益分发等场景。您只需提交包含多条 payout item 的请求，Cobo 将为每条 item 执行地址校验与合规验证（KYA），并根据执行模式（execution mode）决定是否继续链上转账。
...
- payout_params：批量 payout 明细数组，每条包含：
  - token_id
  - receiving_address
  - amount
...
| -------------- | ----------------------------- | -------------------------------------- | ---------------------------------------- |
| Strict         | 只要 **任意一条** payout item 地址有问题 | **不会发起任何链上转账**（整单不执行）                  | 更换有问题的地址后，**重新提交整单 Bulk Send**（需要包含全部条目） |
...
4. 创建新的 bulk send 执行补发

说明：以下是示意代码（Node.js 风格），你们可按实际 SDK/签名方式改写。
...
app.post("/webhooks/cobo", async (req, res) => {
    ...
    const createResp = await fetch(`https://api.example.com/v1/payments/bulk_sends`, {
        ...
        body: JSON.stringify({
            source_account,
            execution_mode: "Partial", // 补发一般建议 Partial，避免再次被单点阻塞（按你们策略调整）
...
- **失败分类处理**：
  ...
  - status=Failed/NotExecuted：建议也换地址（按你给的规则），并保留失败原因用于风控/用户提示