...
- 在创建 Bulk Send 前，请配置 Fee Station 并确保其余额充足。如果未配置 Fee Station 或余额不足，请求会在发起任何转账前被拒绝（例如返回 `fee_station_not_configured` 或 `fee_station_insufficient`）。ERC-20 代币转账通过智能合约执行，需要已充值的 Fee Station 来支付交易手续费；TRON 上的智能合约转账无需 Fee Station。
...
<Note>
  在 `Strict` 模式下，要么全成、要么全不成的保证仅适用于校验阶段。只要有任意条目校验失败，则不会发起任何链上转账；若全部条目校验通过，则所有转账会一并发起。校验通过后，单笔转账仍可能在风控检查或链上执行阶段失败，因此 `Strict` 批次仍可能以 `PartiallyCompleted` 状态结束。对于 `Strict` 批次的 `PartiallyCompleted` 状态，请按照下文重试流程同样处理。
</Note>
