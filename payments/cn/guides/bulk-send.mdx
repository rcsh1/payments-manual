...
在进行批量下发时，Cobo 使用智能合约执行批量转账。无论底层使用智能合约还是普通转账，Cobo 都会对每笔操作向您注册的 Callback Endpoint 发送 Callback 消息，以便您检查详情并批准或拒绝执行。要了解如何设置和注册 Callback Endpoint，请参阅 [Callback 介绍](/payments/cn/developer-tools/callback)。
