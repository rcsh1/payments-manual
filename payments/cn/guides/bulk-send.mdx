...
批量下发指一次性向多个收款地址批量转出加密货币，适用于工资发放、返佣奖励、合作方结算、电商/广告平台商家收益分发等场景。您只需提交包含多条批量下发条目的请求，Cobo 将为每个条目执行地址校验与合规验证（KYA），并根据执行模式（execution mode）决定是否继续链上转账。

对于批量下发，通知分为两个阶段：

- Callback 阶段：对于执行前需要您审批的每一笔操作，Cobo 会先发送 Callback 请求。
- Webhook 阶段：在批次状态发生变化后，Cobo 会发送 `payment.bulk_send.status.updated` 事件。
...
- 如果您需要在执行前审批批量下发操作，请配置 Callback Endpoint。
- 如果您希望在执行开始后自动化状态跟踪和重试，请配置 Webhook Endpoint。
...
Cobo 在收到批量下发请求后，会先对 **请求中所有地址进行校验与验证**（包含格式校验与 KYA 等合规验证）。execution_mode 决定了当**任意地址存在问题**时，Cobo 是否继续对其他正常地址执行链上转账。

#### **Strict vs Partial 的行为差异**

| execution_mode | 校验阶段发现任意地址有问题（KYA 不通过） | 链上转账结果 | 您需要做什么 |
| -------------- | ----------------------------- | -------------------------------------- | ---------------------------------------- |
| Strict         | 只要 **任意一条** 条目地址有问题 | **不会发起任何链上转账**（整单不执行）                  | 更换有问题的地址后，**重新提交整批批量下发任务**（需要包含全部条目） |
| Partial        | 允许存在部分地址有问题                   | 会对 **所有校验通过的地址** 执行链上转账；有问题的地址 **不会转** | 查询失败条目，更换地址后，**仅补发失败条目**（只提交需要补发的条目）  |
...
对于批量下发功能，Callback 请求属于执行前的审批流程。它与 Webhook 事件分离，后者用于在执行开始后报告批次状态。

在 Ethereum 上进行批量下发时，Cobo 使用智能合约执行批量转账；其他链则使用普通交易。无论哪种方式，Cobo 都会对每笔操作向您注册的 Callback Endpoint 发送 Callback 消息，以便您检查详情并批准或拒绝执行。Callback 负载中包含您在批准请求前应核验的操作详情。有关 Callback 的设置，请参阅 [Callback 介绍](/payments/cn/developer-tools/callback)。
...
当批量下发任务的状态切换到以下任一状态时，Cobo 会触发 webhook 事件：

- Completed
- PartiallyCompleted
- Failed

Webhook 通知数据结构如下（批量下发通知示例）：
...
使用 `bulk_send_id` 作为批次资源标识符。如果您在创建批次时提供了 `request_id`，请将其作为业务关联键，在 API 请求、Callback 记录和 Webhook 处理中统一使用。
...
- 使用 `event_id` 对 Webhook 投递去重。对于状态驱动的自动化，请比较 `data.updated_timestamp`，仅处理给定 `bulk_send_id` 的最新状态。

---

### **2.2 收到失败通知后：查询批量下发条目明细**

当收到 PartiallyCompleted 或 Failed 的 webhook 后，使用 **List bulk send items** 接口查询该批量下发任务的每个条目状态：
...
1. 查询条目，定位 ValidationFailed 或 Failed/NotExecuted 的条目
2. 更换这些条目的地址（或修正其它导致失败的参数）
3. **重新提交整批批量下发任务（包含所有条目）**
...
1. 查询条目，筛选失败条目（status=Failed/NotExecuted 或 validation_status=ValidationFailed）
2. 更换失败条目的地址
3. 新建一个批量下发任务，只提交这些失败条目
...
1. 接收 webhook
2. 若状态为 PartiallyCompleted / Failed，则查询条目
...
- **统一使用 `bulk_send_id` 和 `request_id`**：使用 `bulk_send_id` 查询批次和条目状态，使用 `request_id` 将 Cobo 通知映射回您的内部批次记录。
- **处理 Webhook 重放**：使用 `event_id` 对投递去重。对于状态驱动的自动化，请比较 `updated_timestamp`，仅处理给定 `bulk_send_id` 的最新状态。
- **失败分类处理**：
