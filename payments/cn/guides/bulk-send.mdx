...
当 Bulk Send 条目通过上述智能合约批量转账路径执行时，Cobo 会依据您所在团队配置的 Contract Call Transaction Policy 对该条目进行风控评估。如果团队尚未配置 Contract Call Transaction Policy，该条目将默认被拒绝。

在生产环境使用 Bulk Send 前，请配置 Contract Call Transaction Policy。有关风控规则配置的更多说明，请参阅 [交易风控规则介绍](https://manuals.cobo.com/cn/portal/risk-controls/tx-policies-overview)。

若条目因未配置 Contract Call Transaction Policy 而被拒绝，会在失败原因字符串中体现；参见下方 [最佳实践](#4-最佳实践) 中的**失败分类处理**。

某条链上的 Bulk Send 条目是否会通过智能合约批量转账路径执行，从而适用 Contract Call Transaction Policy 而非转账风控规则，取决于您所在团队的具体配置。配置详情请参阅 [交易风控规则介绍](https://manuals.cobo.com/cn/portal/risk-controls/tx-policies-overview)。请勿假定固定的链与策略类型对应关系。
