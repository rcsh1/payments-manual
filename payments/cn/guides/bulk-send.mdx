...
在进行批量下发时，Cobo 使用智能合约执行批量转账。对于每个智能合约操作，Cobo 会向您注册的 Callback Endpoint 发送 Callback 消息，以便您验证合约调用信息并批准或拒绝执行。要了解如何设置和注册 Callback Endpoint，请参阅 [Callback 介绍](/payments/cn/developer-tools/callback)。
