...
<Tabs>
  <Tab title="Payments App" icon="pager">
    ...
    <img
      src="/payments/cn/images/payments/create-payers.png"
    ...
    <img
      src="/payments/cn/images/payments/view-address-button.png"
      className="screenshot_full_screen"
      alt="查看充值地址"
    />
    ...
    <img
      src="/payments/cn/images/payments/view-topup-addresses.png"
      className="screenshot_full_screen"
      alt="查看充值地址"
    />
  ...
  <Tab title="Payments API" icon="code">
    1. 调用 [Create/Get top-up address](/payments/en/api-references/payment/createget-top-up-address) 为付款方创建单一充值地址。请求中的关键参数包括：
    ...
    2. 若需要同时为多个付款方批量创建充值地址，可调用 [Batch create top-up addresses](/payments/en/api-references/payment/batch-create-top-up-addresses) 。
    2. **配置充值事件监听**：通过 Webhook 接收充值相关的事件通知，详情可参考[事件与状态](/payments/cn/guides/status-and-events)。
  </Tab>
</Tabs>

## 获取充值模式相关事件

您可以订阅以下 Webhook 事件，以获取充值地址以及充值交易的实时更新通知。请参考 [Webhook reference](/payments/cn/guides/status-and-events) 了解每个事件的触发时间和返回的数据结构。
...
- **Payments API**：调用 [List payers](/payments/en/api-references/payment/list-payers) 获取所有付款方及其充值交易的详细信息。
...
- Payments API：调用 [Update top-up address](/payments/en/api-references/payment/update-top-up-address) 创建新地址。

<Note>
  对于单个付款方，最多可进行 10 次地址更换，超过次数上限会出现接口请求报错。
</Note>

#### 系统自动更换地址

当某一充值地址出现多次交易无法通过合规筛查的情况时，Cobo 会自动废弃该地址，并为该付款方生成新的充值地址。更换后会触发 `payment.address.updated` 事件，详情可参考[事件与状态](/payments/cn/guides/status-and-events)。
...
当某笔交易收到 `payment.transaction.held` 事件，但长期未收到 `payment.transaction.completed` 事件时，这表明该笔交易未能通过 Cobo KYT 或 Screening App 的合规筛查。这种情况下，您需要按照以下步骤进行处理：
