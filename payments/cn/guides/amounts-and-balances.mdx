...
<Note>此处的余额是可用于资金转出和退款的开发者余额或商户余额，并非收入或利润。佣金费用等费用扣款属于运营余额承担的成本，不代表业务收入减少。</Note>

如需查询余额变动原因，而非仅查询当前总额，请使用 [List balance changes](/payments/en/api-references/payment/list-balance-changes)，并按 `source_type`（例如 `Payout`、`RefundOut` 或 `OrderIn`）或 `source_id` 筛选。
