...
<Warning>Checkout SDK 已进入停用维护期。我们推荐使用更简洁、易用的[付款链接](/payments/cn/guides/payment-link) 方案。现有用户可继续使用 Checkout SDK，但建议尽快迁移至付款链接方案。</Warning>
...
<img src="/payments/cn/images/payments/checkout_sdk.png" className="screenshot_modal" alt="Checkout SDK" />
...
集成 SDK 前，请您完成[前置准备](/payments/cn/guides/preparation)中提到的所有步骤。在前置准备过程中，请确保您已：
...
以下代码片段展示了使用 [Python SDK](/payments/cn/developer-tools/quickstart-python) 获取 Access Token 和 Refresh Token 的示例：
...
2. 在 `src/views/CheckoutDemo.vue` 中进行 iFrame 初始化，并传入商户 ID、金额、币种等订单信息。具体字段解释可参考 [Create Pay-in Order](/payments/en/api-references/payment/create-pay-in-order) 接口文档。
