...
您已完成[前置准备](/payments/cn/guides/preparation)中提到的所有步骤，包括：
...
调用 [Create refund link](/payments/en/api-references/payment/create-refund-link) 接口：
...
<Warning>
  **安全提示**：建议您自行增加2FA验证方式，例如用户需要先输入邮箱/短信验证码后，方可访问退款URL，以防链接泄漏从而资产被误领取。
</Warning>
...
<img
  src="/payments/cn/images/payments/refund-address-collection-default.png"
...
<img
  src="/payments/cn/images/payments/refund-address-collection-change.png"
  className="screenshot_full_screen"
  alt="退款地址收集页面-更改地址"
/>
...
1. Cobo自动使用用户提交的地址信息发起退款任务（链上转账），并通过Webhook通知关键状态，
   ...
   Webhook事件为`payment.refund.status.updated`，包含状态变更：
   - `pending`：退款链接已提交，退款订单已创建
   - `completed`：退款完成
   - `partially.completed`: 部分退款交易已完成，部分退款交易失败。
   - `failed`：退款失败
2. 您也可通过"查询退款"接口按`refund_id`主动拉取状态。

关于Webhook事件的详细信息，请参考[状态与事件](/payments/cn/guides/status-and-events)。
...
<Info>
  退款地址收集URL为一次性链接，用户提交地址后即失效。如需重新收集地址，请调用接口生成新的URL。
</Info>
...
<Note>
  如遇到其他问题，请通过 [help@cobo.com](mailto:help@cobo.com) 联系Cobo支持团队。
</Note>
