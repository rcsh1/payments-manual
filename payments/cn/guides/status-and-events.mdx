...
| `payment.transaction.late` | [**仅适用于订单模式**]充币交易通过合规筛查，但订单已处于终态（`Underpaid`、`Expired` 或 `Completed`）。 | [PaymentTransaction](/payments/cn/guides/status-and-events#paymenttransaction) |
| `payment.transaction.held` | 充币交易正在接受合规筛查。订单模式下，即使订单已达到终态（`Underpaid`、`Expired` 或 `Completed`），该事件仍会被触发。 | [PaymentTransaction](/payments/cn/guides/status-and-events#paymenttransaction) |
| `payment.transaction.failed` | 充币交易合规筛查不通过。 | [PaymentTransaction](/payments/cn/guides/status-and-events#paymenttransaction) |
| `payment.transaction.completed` | 充币交易通过合规筛查，资金已成功入账并计入实收金额。| [PaymentTransaction](/payments/cn/guides/status-and-events#paymenttransaction) |
...
该数据结构包含了 [Get transaction information](https://www.cobo.com/developers/v2/api-references/transactions/get-transaction-information) 响应参数中的所有字段，此外还包含以下字段：
