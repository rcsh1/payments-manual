...
本文介绍如何使用 Cobo Auth 对 Cobo 支付服务的 API 请求进行身份验证。
...
<Info>有关权限和钱包范围的更多详情，请参阅[权限和钱包范围](/payments/cn/developer-tools//permissions-and-scopes)。</Info>
...
- API Key 包含在 API 请求中，使 Cobo 支付服务能够识别客户端。使用前您需要在 Cobo Portal 上注册 API Key。更多信息，请参阅[注册 API Key](https://manuals.cobo.com/cn/portal/developer-console/create-api-key)。
...
Cobo 命令行界面（CLI）是一种强大的开发者工具，旨在帮助您在集成 Payments API 时直接从命令行进行构建、测试和管理。
...
1. 使用以下命令安装 Cobo CLI：
    ...
    有关 Cobo CLI 的系统要求，请参阅[安装 Cobo CLI](/payments/cn/developer-tools/cobo-cli/quick-start-guide#system-requirements)。
...
2. 使用 `keys generate` 命令生成 Ed25519 算法的密钥对，如下所示：
    ...
    生成的密钥将安全存储在您的配置文件中（默认：`~/.cobo/config.toml`）。有关此命令的更多信息，请参阅[密钥管理](/payments/cn/developer-tools/cobo-cli/key-management)。
