...
您可以使用 Webhook 跟踪原始 API 请求返回后的异步支付活动。处理重试时，请使用 `event_id` 作为每次投递事件的幂等键。要将 Webhook 与您系统中的资源关联，请读取该事件类型 `data` 负载中的标识符，例如 `payout_id`、`refund_id`、`bulk_send_id`、`order_id`、`request_id` 或 `psp_order_code`。
...
<Note>使用 `event_id` 对投递去重。使用 `data` 中的标识符将事件与您的资金转出、退款单、批量下发或支付订单记录关联。</Note>

### 必填字段

| 字段 | 类型 | 说明 |
|:-----|:-----|:-----|
| `event_id` | string (UUID) | 事件唯一标识符。使用 `event_id` 对重试推送进行去重，并确保您的处理逻辑只处理每个事件一次。 |
...
| `data` | object | 事件专属负载。使用该对象中的标识符将 Webhook 映射到您系统中的原始资源。事件标识符矩阵和各事件结构请参阅[事件与状态](/payments/cn/guides/status-and-events)。 |
