...
要了解 callback 与 webhook 的区别，请参阅[支付回调](/payments/cn/developer-tools/payment-callbacks)。

## 交易标识符

描述交易的 Webhook 消息体包含两个不同的标识符：

- `transaction_id`：Cobo 为该交易分配的内部标识符。它在交易创建时即生成，并始终包含在消息体中。
- `transaction_hash`：链上交易哈希，通常称为 `txid`。它仅在交易被广播上链后才会填充。在广播之前，该字段为 null 或不存在，因此请勿假定每条消息体中都包含该字段。
