...
```python Python
import cobo_waas2
from cobo_waas2.models.list_subscriptions200_response import (
...
public class Example {
  public static void main(String[] args) {
    ...
    try {
      ListSubscriptions200Response result =
          apiInstance.listSubscriptions(limit, before, after, planId, merchantId, actionId);
...
func main() {
	...
	resp, r, err := apiClient.PaymentAPI.ListSubscriptions(ctx).
		Limit(limit).
		Before(before).
		After(after).
		PlanId(planId).
...
apiInstance.listSubscriptions(opts).then(
  (data) => {
