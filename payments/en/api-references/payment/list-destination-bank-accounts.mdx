...
from cobo_waas2.models.list_destination_bank_accounts200_response import (
...
with cobo_waas2.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        api_response = api_instance.list_destination_bank_accounts(
            limit=limit,
            before=before,
            after=after,
            keyword=keyword,
        ...
        print(
            "Exception when calling PaymentApi->list_destination_bank_accounts: %s\n"
            % e
        )
...
public class Example {
  public static void main(String[] args) {
    ...
    try {
      ListDestinationBankAccounts200Response result =
          apiInstance.listDestinationBankAccounts(
              limit, before, after, keyword, destinationId, bankAccountStatus);
...
func main() {
	...
	resp, r, err := apiClient.PaymentAPI.ListDestinationBankAccounts(ctx).
		Limit(limit).
		Before(before).
		After(after).
		Keyword(keyword).
	...
	if err != nil {
		fmt.Fprintf(
			os.Stderr,
			"Error when calling `PaymentAPI.ListDestinationBankAccounts``: %v\n",
			err,
		)
...
apiInstance.listDestinationBankAccounts(opts).then(
  (data) => {
