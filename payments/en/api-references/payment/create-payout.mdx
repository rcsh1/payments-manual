...
<Note>The `request_id` you provide in this request is returned in `payment.payout.status.updated` webhook payloads together with `payout_id`. Use `request_id` to match the webhook to your original request and `payout_id` to query the payout resource. For status transitions and payload fields, see [Events and status](/payments/en/guides/status-and-events).</Note>
...
with cobo_waas2.ApiClient(configuration) as api_client:
    ...
    create_payout_request = cobo_waas2.CreatePayoutRequest(
        ...
        source_account="req_20260122135755",
        payout_channel="Crypto",
        payout_params=[
            cobo_waas2.PaymentPayoutParam(token_id="ETH_USDT", amount="500.00")
        ],
        recipient_info=cobo_waas2.PaymentPayoutRecipientInfo(
  ...
  public static void main(String[] args) {
    ...
    CreatePayoutRequest createPayoutRequest =
        new CreatePayoutRequest()
            ...
            .sourceAccount("req_20260122135755")
            .payoutChannel(PayoutChannel.CRYPTO)
            .payoutParams(
                Arrays.asList(new PaymentPayoutParam().tokenId("ETH_USDT").amount("500.00")))
            .recipientInfo(new PaymentPayoutRecipientInfo().address("0xabc123456789def0000000000000000000000000").tokenId("ETH_USDT"));
    try {
...
const opts = {
  CreatePayoutRequest: CoboWaas2.CreatePayoutRequest.constructFromObject({
    ...
    source_account: "req_20260122135755",
    payout_channel: "Crypto",
    payout_params: [{ token_id: "ETH_USDT", amount: "500.00" }],
    recipient_info: {
        address:"0xabc123456789def0000000000000000000000000",
        token_id:"ETH_USDT"
    },
