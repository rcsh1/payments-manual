---
openapi: get /payments/subscription_plans/{plan_id}
---

<RequestExample>
```python Python
import cobo_waas2
from cobo_waas2.models.payment_subscription_plan_detail import (
...
with cobo_waas2.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        print(
            "Exception when calling PaymentApi->get_subscription_plan_by_id: %s\n" % e
        )
...
func main() {
	...
	resp, r, err := apiClient.PaymentAPI.GetSubscriptionPlanById(ctx, planId).
		TokenId(tokenId).
		Execute()
	if err != nil {
		fmt.Fprintf(
			os.Stderr,
			"Error when calling `PaymentAPI.GetSubscriptionPlanById``: %v\n",
			err,
		)
...
apiInstance.getSubscriptionPlanById(plan_id, token_id).then(
  (data) => {
