...
public class Example {
  public static void main(String[] args) {
    ...
    try {
      ListDestinations200Response result =
          apiInstance.listDestinations(
              limit, before, after, keyword, destinationType, country, merchantIds);
...
func main() {
	...
	resp, r, err := apiClient.PaymentAPI.ListDestinations(ctx).
		Limit(limit).
		Before(before).
		After(after).
		Keyword(keyword).
...
apiInstance.listDestinations(opts).then(
  (data) => {
