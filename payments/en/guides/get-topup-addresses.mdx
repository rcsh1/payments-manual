...
<Tabs>
  ...
  <Tab title="Payments API" icon="code">
    ...
    2. If you need to create top-up addresses for multiple payers in batches, you can call [Create batch top-up address](/payments/en/api-references/payment/createget-top-up-address).
...
When a top-up address has multiple transactions that fail to pass compliance screening, Cobo will automatically abandon that address and generate a new top-up address for the payer. After replacement, the `payment.address.updated` event will be triggered. For details, refer to [Events and status](/payments/en/guides/status-and-events).
