...
Bulk Send has two notification phases:

- Callback phase: Cobo sends a callback request before executing each operation that requires your approval.
- Webhook phase: Cobo sends `payment.bulk_send.status.updated` events after the batch status changes.
...
- Configure a callback endpoint if you need to approve Bulk Send operations before execution.
- Configure a webhook endpoint if you want to automate status tracking and retries after execution starts.
...
For Bulk Send, callback requests are part of the approval flow before execution. They are separate from webhook events, which report batch status after execution starts.

For Bulk Send on Ethereum, Cobo uses smart contracts to perform batch transfers. For other chains, Cobo uses regular transactions instead. In both cases, Cobo sends a callback message to your registered callback endpoint for each operation so that you can inspect the details and approve or deny execution. The callback payload contains the operation details you should verify before you approve the request. For callback setup, see [Introduction to Callbacks](/payments/en/developer-tools/callback).
...
Use `bulk_send_id` as the batch resource identifier. If you supplied `request_id` when creating the batch, use it as your business correlation key across your API request, callback records, and webhook processing.
...
- Use `event_id` to deduplicate webhook deliveries. For status-driven automation, compare `data.updated_timestamp` and process only the newest state for a given `bulk_send_id`.
...
- **Use `bulk_send_id` and `request_id` consistently**: use `bulk_send_id` to query batch and item status, and use `request_id` to map Cobo notifications back to your internal batch record.
- **Webhook replay handling**: use `event_id` to deduplicate deliveries. For status-driven automation, compare `updated_timestamp` and process only the newest state for a given `bulk_send_id`.
