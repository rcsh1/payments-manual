...
When executing a Bulk Send, Cobo uses smart contracts to perform batch transfers. For each smart contract operation, Cobo sends a callback message to your registered callback endpoint so that you can verify the contract call information and approve or deny execution. To learn how to set up and register a callback endpoint, see [Introduction to Callbacks](/payments/en/developer-tools/callback).
