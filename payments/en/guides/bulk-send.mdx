...
- Configure a Fee Station and make sure it is sufficiently funded before you create a Bulk Send. If the Fee Station is not configured or does not have enough balance, the request is rejected before any transfer is initiated (for example, with `fee_station_not_configured` or `fee_station_insufficient`). ERC-20 token transfers are executed through a smart contract and require a funded Fee Station to cover the transaction fees; smart-contract transfers on TRON do not require a Fee Station.
...
<Note>
  In `Strict` mode, the all-or-nothing guarantee applies at the validation stage only. If any item fails validation, no on-chain transfers are initiated; if all items pass validation, every transfer is initiated together. After validation passes, an individual transfer can still fail during risk-control checks or on-chain execution, so a `Strict` batch can still finish with the `PartiallyCompleted` status. Handle `PartiallyCompleted` for `Strict` batches the same way as described in the retry workflow below.
</Note>
