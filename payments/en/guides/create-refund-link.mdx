...
You have completed all the steps mentioned in [Preparation](/payments/en/guides/preparation), including:
...
Call [Create refund link](/payments/en/api-references/payment/create-refund-link):
...
- `fee_amount`: Optional, refund processing fee. Invalid if source is PSP Balance
...
<Warning>
  **Security reminder**: It is recommended that you add 2FA verification, for example, requiring users to enter an email/SMS verification code before accessing the refund URL, to prevent link leakage and assets being mistakenly claimed.
</Warning>
...
<img
  src="/payments/en/images/payments/refund-address-collection-default.png"
  className="screenshot_full_screen"
  alt="Refund address collection page - default address"
/>

**Change refund address page** (requires filling in new address and Memo):

<img
  src="/payments/en/images/payments/refund-address-collection-change.png"
  className="screenshot_full_screen"
  alt="Refund address collection page - change address"
/>

#### 5. Automatically initiate refund and notification (Cobo backend)

1. Cobo automatically initiates refund tasks (on-chain transactions) using the address information submitted by the user. You can track the progress via Webhook notifications. The relevant event is `payment.refund.status.updated`, which includes the following status changes:
   - `pending`: The refund request has been submitted, and the refund order is successfully created.
   - `completed`: The refund process is complete.
   - `partially.completed`: Some refund transactions have been completed, while others have failed.
   - `failed`: The refund process has failed.
2. You can also actively pull the status through the "Query refund" API by `refund_id`.

For detailed information about Webhook events, refer to [Status and events](/payments/en/guides/status-and-events).
...
<Info>
  The refund address collection URL is a one-time link that becomes invalid after the user submits the address. If you need to collect the address again, please call the API to generate a new URL.
</Info>
...
<Note>
  If you encounter other issues, please contact Cobo's support team through [help@cobo.com](mailto:help@cobo.com).
</Note>
