...
<Warning>Checkout SDK has been deprecated. We recommend using the simpler and more user-friendly [Payment Link](/payments/en/guides/payment-link) solution. Existing users can continue to use Checkout SDK but are advised to migrate to Payment Link.</Warning>
...
<img src="/payments/en/images/payments/checkout_sdk.png" className="screenshot_modal" alt="Checkout SDK" />
...
Before integrating the SDK, please complete all the steps mentioned in [Preparation](/payments/en/guides/preparation). During the preparation process, please ensure that you have:
...
The following code snippet shows an example of using the [Python SDK](/payments/en/developer-tools/quickstart-python) to obtain Access Token and Refresh Token:
...
2. Initialize the iFrame in `src/views/CheckoutDemo.vue` and pass in order information such as merchant ID, amount, and currency. For specific field explanations, refer to the [Create Pay-in Order](/payments/en/api-references/payment/create-pay-in-order) API documentation.
