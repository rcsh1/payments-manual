...
| Settlement Network | `payment.transaction.settlement_network.created` | Related to the creation of a settlement-network payment transaction. For the product workflow, see [Settlement Network](/payments/en/guides/settlement-network). | Follows the `payment.transaction.*` event format. See [Transaction payload fields](#transaction-payload-fields). |
| Settlement Network | `payment.transaction.settlement_network.completed` | Related to the completion of a settlement-network payment transaction. For the product workflow, see [Settlement Network](/payments/en/guides/settlement-network). | Follows the `payment.transaction.*` event format. See [Transaction payload fields](#transaction-payload-fields). |
...
For the Settlement Network product workflow, see [Settlement Network](/payments/en/guides/settlement-network).
