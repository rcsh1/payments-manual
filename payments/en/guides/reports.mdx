...
## Where to generate and download reports

Use [Generate reports](/payments/en/api-references/payment/generate-reports) to request a report. The API key requires the `payment_reports.create` scope. Use [List all reports](/payments/en/api-references/payment/list-all-reports) to retrieve generated report records; this operation requires `payment_reports.read`.

List results are paginated. Set `limit` to control the page size and use either `before` or `after` to move through the result set.
...
For the fee and net-amount mechanics behind the `bridge_fee` column, see [Bridge fee](/payments/en/guides/billing#bridge-fee).
...
For the commission and conversion components to reconcile with this report, see [Off-Ramp fee](/payments/en/guides/billing#off-ramp-fee).
...
For developer and Cobo commission fee behavior, see [Refund fees](/payments/en/guides/billing#refund-fees).
...
For the distinction between Cobo commission fees and developer fees, see [Pay-in](/payments/en/guides/billing#pay-in).
...
For payout commission calculation, estimation, and charging, see [Payout commission fee](/payments/en/guides/billing#payout-commission-fee).
