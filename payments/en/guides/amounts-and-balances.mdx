...
<Note>Balance here means the developer or merchant account balance available for payouts and refunds. It is not a revenue or profit figure. A fee deduction, such as a payout commission fee, is a cost against this operating balance, not a reduction in business revenue.</Note>

To see why a balance changed rather than only its current total, use [List balance changes](/payments/en/api-references/payment/list-balance-changes), filtered by `source_type` (for example, `Payout`, `RefundOut`, or `OrderIn`) or `source_id`.
