...
| 2025 | 403 | Your API key does not have the permission required to perform this action. | Check the OAuth scopes and roles assigned to your API key in Cobo Portal. Refer to [Roles and permissions](/payments/en/guides/roles-and-permissions) for details. |
...
| 30036 | 400 | The fee station balance is insufficient. The fee station needs to be recharged before transfers can proceed. | Deposit the required gas token to your fee station deposit address for the chain, then retry the transfer. |
| 30038 | 400 | The fee station token balance is insufficient to cover the gas fee for this transaction. | Deposit the required gas token to your fee station deposit address for the chain, then retry the transfer. |
| 30039 | 400 | Cobo's gas station balance is temporarily insufficient to cover the gas fee. | Retry the transfer after a short wait. This condition is resolved by Cobo automatically. |
