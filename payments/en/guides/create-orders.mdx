...
[Accounts and fund allocation](/payments/en/guides/amounts-and-balances#deposit-attribution) details how Cobo handles funds in the case of overpayment, underpayment, and late payment — including that a deposit received after an order has already expired, reached the terminal `Underpaid` status, or is `Completed` is credited to the developer account rather than the merchant account.
